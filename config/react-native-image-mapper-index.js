                ...
                >
                    {imgMap.map((item, index) => (
                        <TouchableOpacity
                            ...
                            style={[
                                { position: 'absolute', display: 'flex', alignContent: 'center', justifyContent: 'center', alignItems: 'center' },
                                this.buildStyle(item, index)
                            ]}
                        ><Text style={{ fontSize: 18, color: '#FFF' }}>{item.tableNoForDisplay}</Text></TouchableOpacity>
                    ))}
                </ImageBackground>
            </View>
        );
